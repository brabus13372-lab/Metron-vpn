...
SUPPORT_COOLDOWN = 10.0  # seconds
TG_TEXT_LIMIT = 4096
TG_CAPTION_LIMIT = 1024

_BOT_NAME = BOT_NAME or "MetronVPN"

# TTLCache auto-evicts entries older than SUPPORT_COOLDOWN seconds.
# maxsize=10000 caps memory usage even with many unique users.
_last_support_message: TTLCache = TTLCache(maxsize=10_000, ttl=SUPPORT_COOLDOWN)
...
async def forward_to_admin(message: types.Message, state: FSMContext, bot: Bot) -> None:
    ...
    if user_id in _last_support_message:
    ...
    try:
        if message.text:
            ...
            await bot.send_message(
                ADMIN_ID, full_text, parse_mode="HTML", reply_markup=kb,
            )
        elif message.photo:
...
async def send_admin_reply_to_user(message: types.Message, state: FSMContext, bot: Bot) -> None:
    ...
    try:
        await bot.send_message(
            target_user_id,
            f"✉️ <b>Ответ от поддержки {_BOT_NAME}:</b>",
            parse_mode="HTML",
        )
        await bot.copy_message(
        ...
        logger.info("support.admin_replied target_user_id=%s", target_user_id)

    except Exception:
        logger.exception("support.reply_failed target_user_id=%s", target_user_id)
