...
async def migrate() -> None:
    ...
    try:
        ...
        counts_before["devices"] = sum(1 for user in users if user["uuid"] and user["vless_link"])
        ...
        async with pg_conn.transaction():
            ...
            # ----- users -----
            for user in users:
                expire = safe_parse_datetime(user["trial_expired_at"])
            ...
            for user in users:
                if not user["uuid"] or not user["vless_link"]:
                    continue
