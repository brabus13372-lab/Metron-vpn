...
Использование:
    ...
    # Canonical path:
    export $(grep -v '^#' .env | xargs) && python -m scripts.maintenance.cleanup_orphan_keys --dry-run
"""
...
async def run(apply: bool) -> None:
    ...
    if apply and not PANEL_URL:
        log.error("PANEL_URL not set")
        sys.exit(1)
    if apply and not PANEL_USER:
        log.error("PANEL_USER not set")
        sys.exit(1)
    if apply and not PANEL_PASS:
        log.error("PANEL_PASS not set")
        sys.exit(1)
    ...
    async with httpx.AsyncClient(verify=False) as http:
        if not await _panel_login(http):
            log.error("Cannot proceed without panel auth.")
            sys.exit(1)
        ...
        for orphan in orphans:
            uid = orphan["user_id"]
            account_uuid = orphan["account_uuid"]
            log.info("Processing user_id=%s  uuid=%s ...", uid, account_uuid)

            panel_ok, panel_msg = await _panel_delete_client(http, account_uuid)
